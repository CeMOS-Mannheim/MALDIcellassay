...
Note, that we use the pre-processed spectra (baseline removed). Baseline removal and smoothing can (if needed) be performed with `MALDIquant`.
```{r fitCruve, eval = TRUE}
...
We can also extract a data.frame with all the peaks and quality metrics. We actually got a sneak peak on the results above from the `show` method of the `MALDIassay`-class.
From there we can see that `mzIdx` 197 (**m/z** 811.6) scored the highest CRS score with around 75%.
...
plotPeak(res, mzIdx = 197, tol = 0.1)
plotCurves(res, mzIdx = 197, errorbars = "sd")
```
